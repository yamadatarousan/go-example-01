...
func setupTestRouter(dbConn *sql.DB) *gin.Engine {
	...
	_ = service.NewReminderService(reminderRepo, notificationRepo, todoRepo) // 将来のバックグラウンドワーカー用
	...
	// setupRouter関数を再利用
	return setupRouter(cfg, authService, userHandler, todoHandler, adminHandler, categoryHandler, notificationHandler)
}
