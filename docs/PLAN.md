...
## 大前提
- ユーザー（私）はプログラミングにおける型に興味がある
- 型を中心として知識のネットワークを広げ、知識のモデルを作り上げていきたい
- 可能な限り型を緻密に使ってほしい
- またユーザー（私）は境界の契約に興味がある
- 型の使い方と境界の契約、これを常に良いものとするべく開発に取り組んでほしい
...
- [x] 認証フローE2E（サインアップ→ログイン→/todos）

#### 成果物
- [x] サインアップ → ログイン → TODO一覧へのフローが動く
...
- [x] TODO一覧のUIテスト
- [x] TODO一覧のE2Eテスト（一覧 and 空状態の確認）
- [x] ルート保護 middleware.ts（/todos 実装後に対応）
  - ログイン必須: /todos, /dashboard, /projects, /settings
  - 未ログイン専用（ログイン済みは /todos へ）: /login, /signup
  - 公開: /
  - 以降、保護対象ページ追加時にリスト更新
- [x] ルート保護 middleware.tsのE2Eテスト

#### 成果物
- [x] バックエンドからTODOを取得して一覧表示できる
...
- [ ] バックエンドテストの見直し（空配列の期待など）※/todos が null を返してフロントが落ちた件の再発防止
- [ ] OpenAPIスキーマのnullable見直し（null最小化の方針に合わせる）

---

## 🚨 緊急タスク

**背景**: フロントエンドとバックエンドの境界に契約がなく、バックエンドの想定外の null 返却でフロントがエラーになった。再発防止のため境界の契約を明文化し、実行時にも検証する必要がある。  
**概要**: OpenAPI 3.1 を単一の契約ソースとして導入し、/todos 系から最小範囲で開始。バックエンドは生成型＋リクエスト/レスポンス検証、フロントは生成型への移行で契約遵守を強制する。
...
- **運用ルール（確定事項）**:
  ...
  - OpenAPIバージョン: 3.0.3 を使用（3.1の`type: [string, "null"]`と`nullable`の表現差分により、oapi-codegen/Redoclyの両立が困難だったため）
...
- [X] /todos 系エンドポイントを定義（一覧・詳細・作成・更新・削除・complete/reopen・overdue/today/week・search・statistics）
...
- [X] バックエンド: openapi から Go 型を生成（`oapi-codegen` 等）
- [X] バックエンド: Gin のリクエストバリデーションを有効化（生成ミドルウェアの適用）
- [X] バックエンド: OpenAPIのリクエストボディ検証を有効化（ミドルウェア導入）
- [X] テスト構成方針を決定（エンドポイント単位で分割、TestMainはmain_test.goに残す）
- [X] main_test.go のテストを分割（エンドポイント単位のテストファイルに移動）
- [X] OpenAPIリクエストボディ検証のテストを追加（/todos の不正/正/nullable）
- [X] テスト実行（バックエンド統合テスト）
- [X] レスポンス検証の運用ルールを定義（適用層/対象範囲/違反時挙動/ログ項目）
- [X] バックエンド: レスポンスバリデーションを追加（契約違反/null混入を検出し、ログ化/監視へ）
- [X] フロントエンド: openapi から TS 型を生成（`openapi-typescript` 等）
- [X] フロントエンド: /todos の API 呼び出しを生成型へ切替（手書き型の依存を削減）
- [X] モックデータも契約に合わせて修正（/api/v1/todos/stats → /api/v1/todos/statistics など）
- [X] CI で契約の差分/生成物の未更新を検出（OpenAPIの変更時に生成物が更新されていることを保証）
- [X] ドキュメント更新（開発手順・生成コマンド・運用ルールを明記）
- [X] テスト実行（フロントエンド/バックエンドの統合）
- [X] OpenAPI運用ドキュメントを作成（更新手順・生成物の連動・内部動作の要点）

## ♻️ 継続タスク（運用・保守）

- CI失敗の原因調査・修正（常時、最優先）
- 進行中フェーズの実装よりも、CIを先に復旧する

---

**最終更新**: 2026-01-29
**完了ステータス**: Phase 8.6まで完了
**次のアクション**: Phase 8.7のタスクを進める
