...
---
#### **Gemini提案: Phase 6 実装計画案**

このフェーズの目標は、アプリケーションを本番環境で安定かつ安全に稼働させるための品質向上です。以下の3つの柱で進めます。

##### **1. セキュリティ強化 (6.1)**

1.  **リフレッシュトークンの実装**
    *   **目的**: アクセストークンの有効期限を短縮し（例: 15分）、漏洩時のリスクを低減します。
    *   **方針**:
        1.  **DBマイグレーション**: `users`テーブルまたは新しい`refresh_tokens`テーブルに、リフレッシュトークン（ハッシュ化して）と有効期限を保存するカラムを追加します。
        2.  **Service層の拡張**: `AuthService`に`RefreshAccessToken(refreshToken string)`メソッドを追加します。このメソッドは、受け取ったリフレッシュトークンを検証し、有効であれば新しいアクセストークンを生成します。
        3.  **Handler層の拡張**: `/api/v1/auth/refresh`エンドポイントを追加し、`RefreshAccessToken`サービスを呼び出します。
        4.  **ログイン処理の変更**: ログイン成功時に、アクセストークン（短命）とリフレッシュトークン（長命）の両方を生成し、リフレッシュトークンはDBに保存後、クライアントに返却します。
        5.  **ログアウト処理の変更**: ログアウト時にDBから該当のリフレッシュトークンを削除し、無効化します。

2.  **レート制限の実装**
    *   **目的**: ブルートフォース攻撃やDoS攻撃からAPIサーバーを保護します。
    *   **方針**:
        1.  **ライブラリ導入**: `golang.org/x/time/rate` を利用します。
        2.  **ミドルウェア作成**: IPアドレスごとにリクエスト数を管理するGinミドルウェアを作成します。`map[string]*rate.Limiter` のような構造で、IPアドレスごとのリミッターを保持します。
        3.  **グローバル適用**: 作成したミドルウェアを、ルーターのグローバルミドルウェアとして登録します。

3.  **セキュリティヘッダーの追加**
    *   **目的**: XSSやクリックジャッキングなどの一般的なWeb脆弱性を緩和します。
    *   **方針**:
        1.  **ミドルウェア作成**: `Content-Security-Policy`, `X-Content-Type-Options`, `X-Frame-Options` などのセキュリティ関連ヘッダーをレスポンスに付与する専用のミドルウェアを作成し、グローバルに適用します。`unrolled/secure`のようなライブラリの利用も検討します。

4.  **監査ログの拡充**
    *   **目的**: 誰が・いつ・何をしたかを追跡可能にし、セキュリティインシデントの調査を容易にします。
    *   **方針**:
        1.  **DBマイグレーション**: `todo_audit_logs`テーブルに`action`カラム（例: 'CREATE', 'UPDATE', 'DELETE'）を追加します。
        2.  **Repository層の修正**: `TodoRepository`の`UpdateByID`, `DeleteByID`などのメソッド内で、対応するアクション（'UPDATE', 'DELETE'）を監査ログに記録する処理を追加します。
        3.  **対象拡大**: `Project`や`Comment`など、他の重要なリソースについても同様の監査ログテーブルとロギング処理を実装します。

##### **2. パフォーマンス最適化 (6.2)**

1.  **データベースインデックスの追加**
    *   **目的**: 頻繁な検索条件（WHERE句）やソート（ORDER BY句）で使われるカラムにインデックスを作成し、クエリを高速化します。
    *   **方針**:
        1.  **DBマイグレーション**: `PLAN.md`記載の通り、`todos`テーブルの`(user_id, status)`複合インデックスと、`due_date`のインデックスを追加するマイグレーションファイルを作成します。
        2.  **分析と追加**: `EXPLAIN ANALYZE`を使い、特にフィルタリングやソートが多用されるエンドポイントのクエリを分析し、`projects`や`comments`テーブルなどにも効果的なインデックスを追加します。

2.  **N+1クエリの排除**
    *   **目的**: ループ内でのDBクエリ発行を防ぎ、DBへのラウンドトリップを削減します。
    *   **方針**:
        1.  **問題の特定**: 例えば「プロジェクト一覧を取得し、各プロジェクトのオーナー名を表示する」といった処理で発生しがちです。ログを監視して同様のクエリが連続発行されていないか確認します。
        2.  **クエリ修正**: `JOIN`句を用いて一度のクエリで関連データ（例: ユーザー名）まで取得するようにリポジトリ層のSQLを修正します。
        3.  **データローダーパターンの検討**: 複雑なケースでは`graph-gophers/dataloader`などを導入し、IDをまとめて後から一括で取得する方式を検討します。

3.  **Redisキャッシュの導入**
    *   **目的**: 頻繁に参照されるが更新は少ないデータをメモリ上にキャッシュし、DB負荷を劇的に軽減します。
    *   **方針**:
        1.  **環境構築**: `docker-compose.yml`にRedisサービスを追加します。
        2.  **ライブラリ導入**: `go-redis/redis`を導入し、設定(`config`)と接続処理を`main.go`に追加します。
        3.  **Cache-Asideパターン実装**: `Repository`層を修正します。まずRedisにデータがあるか確認し、なければDBから取得して結果をRedisに保存、次回以降はRedisから返す、というロジックを実装します。
        4.  **キャッシュの無効化**: データが更新・削除された際には、対応するキャッシュをRedisから削除する処理を`Service`層または`Repository`層に追加します。
        5.  **対象候補**: 統計情報(`GET /todos/stats`)、カテゴリー一覧、ユーザー情報など。

##### **3. 監視・ロギング (6.3)**

1.  **構造化ロギング (zap) の導入**
    *   **目的**: ログをJSON形式で出力し、機械的な集計や分析（例: Datadog, Splunk）を容易にします。
    *   **方針**:
        1.  **ライブラリ導入**: `go.uber.org/zap`を導入します。
        2.  **ロガーの初期化**: `main.go`で`zap.Logger`を初期化し、DIコンテナ経由で各レイヤーに渡します。
        3.  **ログ出力の置換**: プロジェクト全体の`log.Printf`等を`logger.Info()`, `logger.Error()`などに置き換えます。
        4.  **コンテキスト情報の追加**: リクエストIDや認証済みユーザーIDなどをログのフィールドに含めるミドルウェアを作成し、リクエストの追跡を容易にします。

2.  **Prometheusメトリクスの実装**
    *   **目的**: アプリケーションの稼働状況を数値で可視化し、監視やアラートの基準とします。
    *   **方針**:
        1.  **ライブラリ導入**: `prometheus/client_golang`を導入します。
        2.  **エンドポイント公開**: `/metrics`エンドポイントを`main.go`で公開します。
        3.  **メトリクス収集**: HTTPリクエスト数やレイテンシ、エラー率などを計測するGinミドルウェアを作成し、適用します。

3.  **ヘルスチェックエンドポイントの実装**
    *   **目的**: ロードバランサーやコンテナオーケストレーションツール（Kubernetesなど）に対し、アプリケーションが正常かを通知します。
    *   **方針**:
        1.  **エンドポイント作成**: `/healthz`エンドポイントを`main.go`で作成します。
        2.  **状態確認**: このハンドラ内で、DBへの接続確認 (`db.PingContext`) やRedisへの接続確認 (`redisClient.Ping`) を行います。
        3.  **レスポンス**: 全てのチェックが成功すれば`200 OK`を、一つでも失敗すれば`503 Service Unavailable`を返します。
